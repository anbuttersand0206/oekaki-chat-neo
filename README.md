...
> **注意（マルチプロセス構成）**: Gunicorn 複数ワーカーなど複数プロセスで動かすと、各プロセスが独立したカウンタを持つためレート制限が実質的に緩くなります。本番でスケールアウトする場合は Redis ベースの共有カウンターへの移行を検討してください（[TODO 8](#8-レートリミッターの外部化) 参照）。
...
#### 4. Username / Email Enumeration 対策
...
#### 5. `SOCIALACCOUNT_LOGIN_ON_GET` のリスク
