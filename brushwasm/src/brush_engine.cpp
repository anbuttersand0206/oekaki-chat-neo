...
// ── Average colour (reads from canvas_ = composite) ───────────────────────────
void BrushEngine::avgColor(float cx,float cy,float r,float& ar,float& ag,float& ab) const {
    ...
    for(int y=y0;y<=y1;y++) for(int x=x0;x<=x1;x++){
        int i=(y*cw_+x)*4;
        sr2+=canvas_[i]; sg+=canvas_[i+1]; sb+=canvas_[i+2]; cnt++;
    }
...
// ── Direct pixel blend with limit (for eraser) ────────────────────────────────
...
// ── strokeBuf blend + composite (for all ink dabs) ───────────────────────────
// Blends (r,g,b,alpha) into strokeBuf_, then composites preStroke_+strokeBuf_*opa
// into canvas_ for this pixel. r/g/b are 0-255.
void BrushEngine::blendPxBuf(int gx,int gy,float r,float g,float b,float alpha,float opa){
    ...
    // 1) Porter-Duff over into strokeBuf_
    ...
    // 2) Composite preStroke_ + strokeBuf_*opa → canvas_
        ...
        for(int gx=x0;gx<=x1;gx++){
            ...
            for(int iy=0;iy<N;iy++){
                ...
                for(int ix=0;ix<N;ix++){
                    ...
                    else { float s=clamp01((sqrtf(d2)-innerR)/edgeW); sum+=1.f-s*s*(3.f-2.f*s); }
                }
            }
...
void BrushEngine::dabPencil(const BrushCfg& cfg, float cx, float cy, float rad, float flow, float opa) {
    float rr=rad*rad; int x0=clampi((int)(cx-rad),0,cw_-1), y0=clampi((int)(cy-rad),0,ch_-1), x1=clampi((int)(cx+rad),0,cw_-1), y1=clampi((int)(cy+rad),0,ch_-1);
...
void BrushEngine::dabCrayon(const BrushCfg& cfg, float cx, float cy, float rad, float flow, float opa) {
    ...
    for(int gy=y0;gy<=y1;gy++) for(int gx=x0;gx<=x1;gx++){
        ...
        if(a<0.005f) continue; int i=(gy*cw_+gx)*4;
        blendPxBuf(gx,gy,lerpf(cfg.color[0],(float)canvas_[i],0.3f),lerpf(cfg.color[1],(float)canvas_[i+1],0.3f),lerpf(cfg.color[2],(float)canvas_[i+2],0.3f),a,opa);
    }
}
void BrushEngine::dabAirbrush(const BrushCfg& cfg, float cx, float cy, float rad, float flow, float opa) {
    float tr=rad*cfg.hardness+std::max(1.f,(rad-rad*cfg.hardness)*0.6f)*3.f, tr2=tr*tr;
    int x0=clampi((int)(cx-tr),0,cw_-1), y0=clampi((int)(cy-tr),0,ch_-1), x1=clampi((int)(cx+tr),0,cw_-1), y1=clampi((int)(cy+tr),0,ch_-1);
    for(int gy=y0;gy<=y1;gy++) for(int gx=x0;gx<=x1;gx++){
        float d2=sqrf(gx-cx)+sqrf(gy-cy); if(d2>tr2) continue;
        float d=sqrtf(d2), cr=rad*cfg.hardness, s=std::max(1.f,(rad-cr)*0.6f);
        float a=(d<=cr?1.f:expf(-0.5f*sqrf((d-cr)/s)))*flow;
...
void BrushEngine::dabWatercolor(const BrushCfg& cfg, float cx, float cy, float rad, float flow, float opa) {
    // avgColor reads from canvas_ which always reflects the current composite
    ...
    int x0=clampi((int)(cx-rad),0,cw_-1), y0=clampi((int)(cy-rad),0,ch_-1), x1=clampi((int)(cx+rad),0,cw_-1), y1=clampi((int)(cy+rad),0,ch_-1);
    for(int gy=y0;gy<=y1;gy++) for(int gx=x0;gx<=x1;gx++){
        float d2=sqrf(gx-cx)+sqrf(gy-cy); if(d2>rr) continue;
        float ni=sqrtf(d2)/rad, a=(expf(-3.f*ni*ni)+expf(-30.f*sqrf(ni-0.7f))*0.35f)*f*clamp01((1.f-ni)*10.f);
...
void BrushEngine::dabOil(const BrushCfg& cfg, float cx, float cy, float rad, float flow, float opa) {
    ...
    float ux=s_.dirX, uy=s_.dirY, qx=-uy, qy=ux, ra=rad*0.55f, rp=rad, h=clamp01(cfg.hardness-cfg.water*0.2f);
    int x0=clampi((int)(cx-rp),0,cw_-1), y0=clampi((int)(cy-rp),0,ch_-1), x1=clampi((int)(cx+rp),0,cw_-1), y1=clampi((int)(cy+rp),0,ch_-1);
    for(int gy=y0;gy<=y1;gy++) for(int gx=x0;gx<=x1;gx++){
        float dx=gx-cx, dy=gy-cy, al=dx*ux+dy*uy, pe=dx*qx+dy*qy, ed=sqrtf(sqrf(al/ra)+sqrf(pe/rp));
...
void BrushEngine::dabPastel(const BrushCfg& cfg, float cx, float cy, float rad, float flow, float opa) {
    ...
    for(int gy=y0;gy<=y1;gy++) for(int gx=x0;gx<=x1;gx++){
        float d2=sqrf(gx-cx)+sqrf(gy-cy); if(d2>=rr) continue;
        float g=paperNoise(gx,gy), a=softAlpha(sqrtf(d2)/rad,cfg.hardness)*((1.f-g)*0.65f+g*0.35f)*flow*0.7f;
        blendPxBuf(gx,gy,cfg.color[0],cfg.color[1],cfg.color[2],a,opa);
    }
}
void BrushEngine::dabBlur(const BrushCfg& cfg, float cx, float cy, float rad, float flow) {
    int b=std::max(1,(int)(rad*0.2f)); float rr=rad*rad;
    int x0=clampi((int)(cx-rad),0,cw_-1), y0=clampi((int)(cy-rad),0,ch_-1), x1=clampi((int)(cx+rad),0,cw_-1), y1=clampi((int)(cy+rad),0,ch_-1);
    for(int gy=y0;gy<=y1;gy++) for(int gx=x0;gx<=x1;gx++){
        float d2=sqrf(gx-cx)+sqrf(gy-cy); if(d2>=rr) continue;
        float s=softAlpha(sqrtf(d2)/rad,0.f)*flow; if(s<0.005f) continue;
        int i=gy*cw_+gx; if(s<=alphaBuf_[i]) continue; alphaBuf_[i]=s;
        float sr=0,sg=0,sb=0,sa=0; int c=0;
        for(int ky=gy-b;ky<=gy+b;ky++) for(int kx=gx-b;kx<=gx+b;kx++){
            if(ky>=0&&ky<ch_&&kx>=0&&kx<cw_){ int k=(ky*cw_+kx)*4; sr+=preStroke_[k]; sg+=preStroke_[k+1]; sb+=preStroke_[k+2]; sa+=preStroke_[k+3]; c++; }
        }
        if(c){ int idx=i*4; canvas_[idx]=lerpf(preStroke_[idx],sr/c,s); canvas_[idx+1]=lerpf(preStroke_[idx+1],sg/c,s); canvas_[idx+2]=lerpf(preStroke_[idx+2],sb/c,s); canvas_[idx+3]=lerpf(preStroke_[idx+3],sa/c,s); }
    }
    (void)cfg;
}
